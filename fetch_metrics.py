...
def fetch_analytics_metrics(creds, video_ids):
    ...
    # Query each video individually to avoid unsupported query combos
    durations = []
    retentions = []
    video_views = []
    ...
    for vid_id in video_ids:
        try:
            ...
            rows = resp.get('rows', [])
            if rows:
                # Column order: [views, averageViewDuration, averageViewPercentage]
                views = rows[0][0]
    ...
    for vid_id in video_ids:
        try:
            ...
            if rows:
                ...
                total_clicks += impressions * ctr
        except Exception as e:
            print(f'   Warning: Could not fetch CTR for {vid_id}: {e}')
